...
if [[ -f "$OPENAPI" ]]; then
  ...
  for operation in listBlueprints inspectBlueprint generateRepositoryPlan validateRepository; do
    grep -q "operationId: $operation" "$OPENAPI" || fail "OpenAPI sem operationId $operation"
  done
fi

if [[ -f "$POSTMAN" ]]; then
  for request in "List blueprints" "Inspect blueprint" "Generate repository plan" "Validate repository"; do
    grep -q "\"name\": \"$request\"" "$POSTMAN" || fail "Postman sem request $request"
  done
fi

if [[ -f "$MCP" ]]; then
  for tool in list_templates inspect_template generate_repository_plan validate_repository search_pillars search_governance; do
